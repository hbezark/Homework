...
# People you worked with: Emma Welch
...
def string_manip(s):
    s = s.strip() #Remove any leading and trailing spaces from the input string.
    s = s.upper() #Convert the input string to uppercase.
    ...
    if len(s) == 1:   #If the remaining string is a single character just return that string, otherwise, return the string in reverse
        return s
    else:
        return s[::-1] 
...
#when you call .items on a dictionary, it returns a list of tuples
